...
compile_zokrates_code() {
    ...
    time zokrates compile -i "deregister-$version.zok" -c bls12_381 $debug_flag
    cd ..  # Change back to the original directory
}

compute_witness() {
    time zokrates compute-witness --abi --verbose --stdin < "./../../../tests/python/output_files/deregister$initialised-$version-inputs.json"
}
